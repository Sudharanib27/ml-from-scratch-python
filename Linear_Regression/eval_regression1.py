...
# ### Evalvation Model1 : Predict petal_width based on petal_length and sepal_width
# 
...
params = np.load("model1_weights_regularized.npy", allow_pickle=True).item()
weights = params["weights"]
bias = params["bias"]

#load the weights and bias into the model
lr_model1.load_model_parameters(weights, bias)

# Calculate mean squared error
mse = lr_model1.score(X_test, y_test)

# Print the result
print("Mean Squared Error for regularized Model 1:", mse)
...
lr_model1_nonreg = LinearRegression()

# load the saved trained weights and bias parameters
params = np.load("model1_weights_non_regularized.npy", allow_pickle=True).item()
weights = params["weights"]
bias = params["bias"]

#load the weights and bias into the model
lr_model1_nonreg.load_model_parameters(weights, bias)

# Calculate mean squared error
mse = lr_model1_nonreg.score(X_test, y_test)

# Print the result
print("Mean Squared Error for non regularized Model 1:", mse)
