...
# ### Training Model2 : Predict sepal_length based on petal_length and petal_width
# 
...
sns.scatterplot(data=df, x="petal_length", y="petal_width",hue ='sepal_length')
...
lr_model2_non_reg = LinearRegression(max_epochs = 100, learning_rate = 0.01, regularization=0)
lr_model2_non_reg.fit( X_train, y_train)

#saving model weights and bias
lr_model2_non_reg.save_model_parameters("model2_weights_non_regularized.npy")

#Plot the loss graph
plt.plot(range(len(lr_model2_non_reg.mse_loss_history)), np.ravel(lr_model2_non_reg.mse_loss_history), label='Training Loss')
...
lr_model2_reg = LinearRegression(max_epochs = 100, learning_rate = 0.01, regularization=0.1)
lr_model2_reg.fit( X_train, y_train)

#saving model weights and bias
lr_model2_reg.save_model_parameters("model2_weights_regularized.npy")

#Plot the loss graph
plt.plot(range(len(lr_model2_reg.mse_loss_history)), np.ravel(lr_model2_reg.mse_loss_history), label='Training Loss')
plt.xlabel('Step Number')
plt.ylabel('Loss')
plt.title('Training Loss Over Steps')
plt.legend()
...
weight_difference = lr_model2_reg.weights - lr_model2_non_reg.weights
print('non regularized weights value : ',lr_model2_non_reg.weights)
print('regularized weights value     : ',lr_model2_reg.weights)
