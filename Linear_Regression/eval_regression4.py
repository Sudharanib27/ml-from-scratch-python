...
# ### Evalvation Model4 : Predict petal_length based on sepal_length and petal_width
# 
...
params = np.load("model4_weights_regularized.npy", allow_pickle=True).item()
weights = params["weights"]
bias = params["bias"]

#load the weights and bias into the model
lr_model4.load_model_parameters(weights, bias)

# Calculate mean squared error
mse = lr_model4.score(X_test, y_test)
...
lr_model4_nonreg = LinearRegression()

# load the saved trained weights and bias parameters
params = np.load("model4_weights_non_regularized.npy", allow_pickle=True).item()
weights = params["weights"]
bias = params["bias"]

#load the weights and bias into the model
lr_model4_nonreg.load_model_parameters(weights, bias)

# Calculate mean squared error
mse = lr_model4_nonreg.score(X_test, y_test)

# Print the result
print("Mean Squared Error for non regularized Model 4:", mse)
