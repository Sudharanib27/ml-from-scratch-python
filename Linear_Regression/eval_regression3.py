...
# ### Evalvation Model3 : Predict sepal_width based on sepal_length and petal_width
# 
...
params = np.load("model3_weights_regularized.npy", allow_pickle=True).item()
weights = params["weights"]
bias = params["bias"]

lr_model3.load_model_parameters(weights, bias)

# Calculate mean squared error
mse = lr_model3.score(X_test, y_test)
...
lr_model3_nonreg = LinearRegression()

# load the saved trained weights and bias parameters
params = np.load("model3_weights_non_regularized.npy", allow_pickle=True).item()
weights = params["weights"]
bias = params["bias"]

#load the weights and bias into the model
lr_model3_nonreg.load_model_parameters(weights, bias)

# Calculate mean squared error
mse = lr_model3_nonreg.score(X_test, y_test)

# Print the result
print("Mean Squared Error for non regularized Model 3:", mse)
